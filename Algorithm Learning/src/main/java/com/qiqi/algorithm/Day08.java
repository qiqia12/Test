...
public class Day08 {
   ...
   public class Node1 {
    ...
    public int maxSumBST(TreeNode root) {
        list = new PriorityQueue<>((a,b)->b-a);
        pro(root);
        return Math.max(0,list.peek());
    }

    private Node pro(TreeNode root) {
        if (root == null) return null;
        Node leftNode = pro(root.left);
        Node rightNode = pro(root.right);
        if (leftNode == null && rightNode == null){
            list.add(root.val);
            return new Node(true,root.val,root.val,root.val);
        }
        if (leftNode == null){
            if (root.val < rightNode.max && root.val < rightNode.min){
                list.add(root.val+ rightNode.res);
                return new Node(true, root.val, rightNode.max, root.val+ rightNode.res);
            }else{
